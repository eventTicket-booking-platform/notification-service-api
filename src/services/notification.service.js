...
class NotificationService {
    ...
    switch (type) {
      ...
      case 'BOOKING_CONFIRMED': {
        const bookingDate = payload.bookingDate
          ? new Date(payload.bookingDate).toLocaleString('en-US', { timeZone: 'UTC' })
          : 'N/A';
  ...
  async retryFailedNotification(notificationId) {
    ...
    try {
      const providerResponse = await brevoEmailService.sendTransactionalEmail({
        ...
        htmlContent: this.buildEmailContent(notification.type, notification.metadata).html
      });
      ...
      notification.status = 'FAILED';
      notification.failureReason =
        error?.details?.message || error?.message || 'Notification retry failed';
      await notification.save();
      throw error;
    }
  }
  ...
  async getStats() {
    const [totalNotifications, sentCount, failedCount, unreadCount] = await Promise.all([
