...
class AdminNotificationController {
  async getLogs(req, res, next) {
    try {
      ...
      return res.status(200).json(successResponse(200, 'Notification logs fetched successfully', result));
    } catch (error) {
      return next(error);
    }
  }
  ...
  async getFailedNotifications(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json(successResponse(200, 'Failed notifications fetched successfully', notifications));
    } catch (error) {
      return next(error);
    }
  }
  ...
  async retryFailedNotification(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json(successResponse(200, 'Notification retried successfully', notification));
    } catch (error) {
      return next(error);
    }
  }
  ...
  async getStats(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json(successResponse(200, 'Notification stats fetched successfully', stats));
    } catch (error) {
      return next(error);
    }
  }
